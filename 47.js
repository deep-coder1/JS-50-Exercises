...
// argument's words. Iÿ any word in the argument is not equal to "zero" or
...
function textToNumberBinary(str){
    const words = str.split(' ').filter(word=>
        word.toLowerCase() === "zero" || word.toLowerCase() === "one");

        console.log(words);

        let binaryString = words.map((word)=>{
            return word.toLowerCase() === "one" ?
            "1" : "0"
        }).join('');
...
// console.log(textToNumberBinary("Zero one one zero ONE zero one zero one"));
console.log(textToNumberBinary("Zero two one zero ONE zero one zero one"));

// Output:- 
// (8) ['Zero', 'one', 'zero', 'ONE', 'zero', 'one', 'zero', 'one']
// 47.js:35 01010101